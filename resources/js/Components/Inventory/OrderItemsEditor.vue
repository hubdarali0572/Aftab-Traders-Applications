...
const stockWarning = (row) => {
    if (props.orderStatus !== 'completed' || !props.warehouseId || !row.product_id) return null;
    const qty = parseFloat(row.quantity) || 0;
...
<template>
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <h3 class="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">{{ $t('Order Items') }}</h3>
        ...
        >
            {{ $t('Completed orders deduct stock from the selected warehouse immediately.') }}
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[1050px]">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    <tr v-for="(row, index) in items" :key="index" class="theme-table-row align-top">
                        ...
                        <td class="px-4 py-2">
                            <select v-model="row.unit_id" class="theme-form-input w-full" required>
