...
const orderLine = (productId) => props.orderDetails.find((d) => String(d.product_id) === String(productId));
...
const lineTotal = (row) => {
    const line = orderLine(row.product_id);
    if (!line || !row.quantity) return '0.00';
    const ordered = parseFloat(line.quantity) || 1;
    const qty = parseFloat(row.quantity) || 0;
    const unitDiscount = (parseFloat(line.discount) || 0) / ordered;
    const unitTax = (parseFloat(line.tax) || 0) / ordered;
    const price = parseFloat(line.unit_price) || 0;
    return (qty * price - unitDiscount * qty + unitTax * qty).toFixed(2);
};

const returnableQty = (productId) => orderLine(productId)?.returnable_qty ?? orderLine(productId)?.quantity ?? 0;

const unitLabel = (row) => orderLine(row.product_id)?.unit?.name ?? orderLine(row.product_id)?.unit?.short_name ?? '—';
...
<template>
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <h3 class="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">{{ $t('Returned Products') }}</h3>
            <button type="button" @click="addRow" class="theme-btn-primary px-4 py-2 text-xs shrink-0">{{ $t('Add Item') }}</button>
        </div>

        <div v-if="!orderDetails.length" class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
            {{ $t('Select an original order to load returnable products.') }}
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[900px]">
                <thead>
                    <tr class="theme-table-header">
                        ...
                        <th class="theme-table-header-cell">{{ $t('Ordered Qty') }}</th>
                        <th class="theme-table-header-cell">{{ $t('Returnable') }}</th>
                        <th class="theme-table-header-cell">{{ $t('Returned Quantity') }}</th>
                    ...
                    <tr v-for="(row, index) in items" :key="index" class="theme-table-row align-top">
                        <td class="px-4 py-2 min-w-[180px]">
                            <select v-model="row.product_id" class="theme-form-input w-full" required @change="onProductChange(index)">
                                ...
                                <option v-for="d in orderDetails" :key="d.product_id" :value="d.product_id" :disabled="parseFloat(d.returnable_qty ?? d.quantity) <= 0">
                        ...
                        <td class="px-4 py-2 text-sm">{{ orderLine(row.product_id)?.quantity ?? '—' }}</td>
                        ...
                        <td class="px-4 py-2 text-sm">{{ unitLabel(row) }}</td>
                        <td class="px-4 py-2 text-sm">${{ orderLine(row.product_id)?.unit_price ?? '0.00' }}</td>
        ...
        <div class="flex flex-wrap gap-6 justify-end text-sm font-bold text-slate-600 dark:text-slate-300">
            <span>{{ $t('Total Returned Quantity') }}: <strong class="text-indigo-600">{{ totalQuantity.toFixed(2) }}</strong></span>
            <span>{{ $t('Total Return Amount') }}: <strong class="text-emerald-600">${{ totalAmount.toFixed(2) }}</strong></span>
        </div>
