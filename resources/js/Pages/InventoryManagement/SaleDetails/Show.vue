    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('sale-details.edit', detail.id)" class="theme-btn-primary px-6 py-2">{{ $t('Edit') }}</Link>
                <Link :href="route('sale-details.index')" class="theme-form-back-link">{{ $t('Back') }}</Link>
            </div>
        </div>

        <div class="theme-form-card p-10 mb-6">
            <h3 class="text-xs font-black uppercase text-slate-400 mb-6 tracking-widest">{{ $t('Sale Information') }}</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Invoice') }}</dt><dd class="font-bold text-indigo-600">{{ detail.sale?.invoice_no }}</dd></div>
                <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Customer') }}</dt><dd class="font-bold">{{ detail.sale?.customer?.customer_name || 'Walk-in' }}</dd></div>
                <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Warehouse') }}</dt><dd class="font-bold">{{ detail.sale?.warehouse?.name }}</dd></div>
            </div>
