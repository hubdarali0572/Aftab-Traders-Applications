...
const confirmDelete = () => {
    if (selectedId.value) {
        router.delete(route('purchase-returns.destroy', selectedId.value), {
...
const applySearch = () => {
    router.get(route('purchase-returns.index'), { search: searchQuery.value || undefined }, { preserveState: true, replace: true });
};
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">Purchase Returns</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">Manage returns to suppliers.</p>
            </div>
            <Link :href="route('purchase-returns.create')" class="theme-btn-primary">
        ...
        <div class="theme-table-card">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                <form @submit.prevent="applySearch" class="flex flex-col sm:flex-row gap-3">
                    <input v-model="searchQuery" type="text" class="theme-form-input flex-1" placeholder="Search by reference or PO #..." />
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="theme-table-header">
                            ...
                            <th class="theme-table-header-cell">Purchase</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        ...
                        <tr v-if="returns.data.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400 font-medium">No purchase return records found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <ConfirmModal :show="isModalOpen" title="Delete Purchase Return" message="Are you sure? All return line items will be reversed from stock." @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
