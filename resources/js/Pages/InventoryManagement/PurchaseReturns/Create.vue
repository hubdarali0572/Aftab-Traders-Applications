...
<template>
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-5 flex justify-between items-center">
            <h2 class="text-2xl font-black text-slate-800">{{ $t('New Purchase Return') }}</h2>
            <Link :href="route('purchase-returns.index')" class="theme-form-back-link">{{ $t('Back to List') }}</Link>
        </div>
        ...
        <form @submit.prevent="submit" class="theme-form-card p-8 md:p-10 space-y-6">
            ...
            <div class="flex justify-center"><PrimaryButton :disabled="form.processing">{{ $t('Save Return') }}</PrimaryButton></div>
        </form>
    </AuthenticatedLayout>
</template>
