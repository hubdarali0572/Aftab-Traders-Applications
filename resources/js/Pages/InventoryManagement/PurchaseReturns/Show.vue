    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('purchase-returns.edit', purchaseReturn.id)" class="theme-btn-primary px-6 py-2 rounded-full">Edit</Link>
                <Link :href="route('purchase-returns.index')" class="theme-form-back-link">Back</Link>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Purchase</dt><dd class="font-bold text-indigo-600">{{ purchaseReturn.purchase?.purchase_no }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Return Date</dt><dd class="font-bold text-slate-700">{{ purchaseReturn.return_date }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Warehouse</dt><dd class="font-bold">{{ purchaseReturn.warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Created By</dt><dd class="font-bold text-slate-700">{{ purchaseReturn.user?.name || '—' }}</dd></div>
                </div>
            </div>
            ...
            <div class="theme-form-card p-8" v-if="purchaseReturn.details?.length">
                <h3 class="text-sm font-black uppercase tracking-widest text-slate-500 mb-4">Return Line Items</h3>
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead><tr class="theme-table-header"><th class="theme-table-header-cell">Product</th><th class="theme-table-header-cell text-right">Qty</th><th class="theme-table-header-cell text-right">Unit Price</th><th class="theme-table-header-cell text-right">Total</th></tr></thead>
