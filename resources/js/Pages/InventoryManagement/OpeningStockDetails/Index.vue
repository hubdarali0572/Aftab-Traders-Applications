...
const confirmDelete = () => {
    router.delete(route('opening-stock-details.destroy', selectedId.value), {
        onSuccess: () => isModalOpen.value = false
    });
};

const applySearch = () => {
    router.get(route('opening-stock-details.index'), { search: searchQuery.value || undefined }, { 
        preserveState: true,
        replace: true 
    });
};
...
<template>
    <AuthenticatedLayout>
        <Head title="Opening Stock Items" />
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="item in details.data" :key="item.id" class="theme-table-row group">
                            <td class="px-6 py-4 text-sm font-bold text-indigo-600 dark:text-indigo-400">
                                {{ item.opening_stock?.reference_no }}
                            </td>
                            <td class="px-6 py-4 text-sm font-medium text-slate-700 dark:text-slate-200">
                                {{ item.product?.name }}
                            </td>
                            <td class="px-6 py-4 text-xs text-slate-500 dark:text-slate-400">
                                <span class="block">B: {{ item.batch_no || '—' }}</span>
                                <span class="block">S: {{ item.serial_no || '—' }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm text-right font-bold text-slate-700 dark:text-slate-200">
                                {{ item.quantity }}
                            </td>
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                <div class="theme-table-actions">
                                    <!-- Show -->
                                    <Link :href="route('opening-stock-details.show', item.id)" class="theme-table-action-btn" title="View Detail">
        ...
        <ConfirmModal 
            ...
            message="Are you sure you want to remove this product from the opening stock? This action cannot be undone." 
