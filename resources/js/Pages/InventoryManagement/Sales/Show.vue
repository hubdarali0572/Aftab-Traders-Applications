    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('sales.edit', sale.id)" class="theme-btn-primary px-6 py-2 rounded-full">{{ $t('Edit') }}</Link>
                <Link :href="route('sale-details.index', { sale_id: sale.id })" class="theme-btn-primary px-6 py-2 rounded-full bg-emerald-600 hover:bg-emerald-700">{{ $t('Line Items') }}</Link>
                <Link :href="route('sales.index')" class="theme-form-back-link">{{ $t('Back') }}</Link>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Customer') }}</dt><dd class="font-bold">{{ sale.customer?.customer_name || 'Walk-in' }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Warehouse') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ sale.warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Sale Date') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ sale.sale_date }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Payment Method') }}</dt><dd class="font-bold uppercase text-indigo-600">{{ sale.payment_method }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Sale Type') }}</dt><dd class="font-bold">{{ sale.sale_type?.replace(/_/g, ' ') }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Created By') }}</dt><dd class="font-bold text-slate-700">{{ sale.user?.name || '—' }}</dd></div>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="theme-form-card p-8 text-center">
                    <dt class="text-xs font-black uppercase text-slate-400 mb-2">{{ $t('Subtotal') }}</dt>
                    <dd class="text-2xl font-black text-slate-700">${{ sale.subtotal }}</dd>
                </div>
                <div class="theme-form-card p-8 text-center">
                    <dt class="text-xs font-black uppercase text-slate-400 mb-2">{{ $t('Discount') }}</dt>
                ...
                <div class="theme-form-card p-8 text-center bg-indigo-50 dark:bg-indigo-900/10">
                    <dt class="text-xs font-black uppercase text-indigo-400 mb-2">{{ $t('Grand Total') }}</dt>
                ...
                <div class="theme-form-card p-8 text-center" :class="sale.due_amount > 0 ? 'bg-rose-50 dark:bg-rose-900/10' : 'bg-emerald-50 dark:bg-emerald-900/10'">
                    <dt class="text-xs font-black uppercase mb-2" :class="sale.due_amount > 0 ? 'text-rose-400' : 'text-emerald-400'">{{ $t('Due Amount') }}</dt>
            ...
            <div class="theme-form-card" v-if="sale.details?.length">
                <div class="p-8 md:p-10">
                    <h3 class="text-sm font-black uppercase tracking-widest text-slate-500 mb-6">{{ $t('Line Items') }}</h3>
