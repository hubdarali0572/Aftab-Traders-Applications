...
const overviewStats = computed(() => [
    { title: 'Total Sales', value: props.summary?.total_sales ?? 0, tone: 'text-indigo-700 dark:text-indigo-300', bg: 'bg-indigo-50 dark:bg-indigo-500/10' },
    { title: 'Total Sales Amount', value: money(props.summary?.total_sales_amount), tone: 'text-emerald-700 dark:text-emerald-300', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
    { title: 'Total Sales Returns', value: props.summary?.total_sales_returns ?? 0, tone: 'text-violet-700 dark:text-violet-300', bg: 'bg-violet-50 dark:bg-violet-500/10' },
    { title: 'Total Return Amount', value: money(props.summary?.total_return_amount), tone: 'text-amber-700 dark:text-amber-300', bg: 'bg-amber-50 dark:bg-amber-500/10' },
    { title: 'Total Products Sold', value: props.summary?.total_products_sold ?? 0, tone: 'text-sky-700 dark:text-sky-300', bg: 'bg-sky-50 dark:bg-sky-500/10' },
    { title: 'Total Outstanding Balance', value: money(props.summary?.total_outstanding), tone: 'text-rose-700 dark:text-rose-300', bg: 'bg-rose-50 dark:bg-rose-500/10' },
]);
...
const confirmDelete = () => {
    if (selectedId.value) {
        router.delete(route('sales.destroy', selectedId.value), { onSuccess: () => closeModal(), onFinish: () => closeModal() });
    }
};
...
const clearSearch = () => { searchQuery.value = ''; saleStatus.value = ''; applySearch(); };
</script>
    ...
    <AuthenticatedLayout>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">{{ $t('Manage sales invoices with items, stock, and customer ledger in one workflow.') }}</p>
            </div>
            <Link :href="route('sales.create')" class="theme-btn-primary shrink-0 inline-flex items-center">
        ...
        <div class="theme-table-card mb-6 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                <h3 class="text-sm font-black uppercase tracking-widest text-slate-600 dark:text-slate-300">{{ $t('Sales Dashboard') }}</h3>
            </div>
            <div class="p-6 grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                <div v-for="stat in overviewStats" :key="stat.title" class="p-4 rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800/50">
                    ...
                    <p class="text-lg font-black mt-1 truncate" :class="stat.tone">{{ stat.value }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="theme-table-card">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                <form @submit.prevent="applySearch" class="flex flex-col md:flex-row gap-3">
                    <input v-model="searchQuery" type="text" class="theme-form-input flex-1" :placeholder="$t('Search by invoice or customer...')" />
            ...
            <div class="theme-table-footer flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="text-[11px] font-bold text-indigo-700 uppercase tracking-widest">{{ $t('Showing') }} {{ sales.from || 0 }} {{ $t('to') }} {{ sales.to || 0 }} {{ $t('of') }} {{ sales.total }} {{ $t('entries') }}</div>
        ...
        <ConfirmModal :show="isModalOpen" :title="$t('Delete Sale')" :message="$t('Are you sure? Stock and ledger entries will be reversed.')" @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
