...
<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">Sales</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">Manage sales invoices and orders.</p>
            </div>
            ...
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                <form @submit.prevent="applySearch" class="flex flex-col md:flex-row gap-3">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button v-if="filters?.search || filters?.sale_status" type="button" @click="clearSearch" class="theme-form-back-link px-4 py-2.5">Clear</button>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="sale in sales.data" :key="sale.id" class="theme-table-row group">
                            ...
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="theme-table-actions">
                                    <Link :href="route('sales.show', sale.id)" class="theme-table-action-btn" title="View">
                                    ...
                                    <Link :href="route('sales.edit', sale.id)" class="theme-table-action-btn theme-table-action-edit" title="Edit">
                                    ...
                                    <button @click="openDeleteModal(sale.id)" class="theme-table-action-btn theme-table-action-delete" title="Delete">
                        ...
                        <tr v-if="sales.data.length === 0">
                            <td colspan="8" class="px-6 py-12 text-center text-slate-400 font-medium">No sales records found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
