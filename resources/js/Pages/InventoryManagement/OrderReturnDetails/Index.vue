...
const props = defineProps({ details: Object, orderReturns: Array, filters: Object });
const page = usePage();
...
const orderReturnId = ref(props.filters?.order_return_id ?? '');
const isModalOpen = ref(false);
const selectedId = ref(null);

const openDeleteModal = (id) => { selectedId.value = id; isModalOpen.value = true; };
const confirmDelete = () => {
    router.delete(route('order-return-details.destroy', selectedId.value), { onSuccess: () => { isModalOpen.value = false; } });
};

const applySearch = () => {
    router.get(route('order-return-details.index'), {
        search: searchQuery.value || undefined,
        order_return_id: orderReturnId.value || undefined,
    }, { preserveState: true, replace: true });
};

const clearSearch = () => { searchQuery.value = ''; orderReturnId.value = ''; applySearch(); };
</script>

<template>
    <AuthenticatedLayout>
        <Head title="Order Return Details" />

        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">Order Return Details</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">Manage returned products against customer orders.</p>
            </div>
            <Link :href="route('order-return-details.create')" class="theme-btn-primary">
        ...
        <div class="theme-table-card">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50">
                <form @submit.prevent="applySearch" class="flex flex-col md:flex-row gap-4">
                    <input v-model="searchQuery" type="text" class="theme-form-input flex-1" placeholder="Search by product, return ref, or order #..." />
                    <select v-model="orderReturnId" class="theme-form-input w-full md:w-64">
                        <option value="">All Returns</option>
                        <option v-for="item in orderReturns" :key="item.id" :value="item.id">{{ item.reference_no }}</option>
                    </select>
                    <div class="flex gap-2">
                        <button type="submit" class="theme-btn-primary px-6 py-2.5">Filter</button>
                        <button v-if="filters?.search || filters?.order_return_id" type="button" @click="clearSearch" class="theme-form-back-link px-4 py-2.5">Clear</button>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="item in details.data" :key="item.id" class="theme-table-row group">
                            <td class="px-6 py-4 font-bold text-indigo-600">{{ item.order_return?.reference_no }}</td>
                            <td class="px-6 py-4 text-sm font-bold text-slate-700 dark:text-slate-300">{{ item.order_return?.order?.order_no }}</td>
                            <td class="px-6 py-4 font-bold text-slate-800 dark:text-slate-200">{{ item.product?.name }}</td>
                            <td class="px-6 py-4 text-right text-sm text-slate-700 dark:text-slate-300">{{ item.quantity }}</td>
                            <td class="px-6 py-4 text-right text-sm text-slate-600 dark:text-slate-400">${{ item.unit_price }}</td>
                        ...
                        <tr v-if="details.data.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400 font-medium">No return line items found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <ConfirmModal :show="isModalOpen" title="Remove Return Item" message="Remove this item from the order return?" @close="isModalOpen = false" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
