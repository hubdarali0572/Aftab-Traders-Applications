...
const applySearch = () => {
    router.get(route('stocks.index'), { search: searchQuery.value || undefined }, {
        preserveState: true,
        replace: true,
    });
};
...
<template>
    <AuthenticatedLayout>
        <Head :title="$t('Stock Management')" />

        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">{{ $t('Stock Management') }}</h2>
            ...
            <Link :href="route('stocks.create')" class="theme-btn-primary">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="stock in stocks.data" :key="stock.id" class="theme-table-row group">
                            ...
                            <td class="px-6 py-2 whitespace-nowrap text-right">
                                <div class="theme-table-actions">
                                    <Link :href="route('stocks.show', stock.id)" class="theme-table-action-btn" title="View Stock">
                                    ...
                                    <Link :href="route('stocks.edit', stock.id)" class="theme-table-action-btn theme-table-action-edit" :title="$t('Edit Thresholds')">
                        ...
                        <tr v-if="stocks.data.length === 0">
                            <td colspan="8" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">{{ $t('No stock records found.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
