...
<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">Stock Transfer Details</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">Manage products within stock transfers.</p>
            </div>
            ...
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50">
                <form @submit.prevent="applySearch" class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button v-if="filters?.search || filters?.transfer_id" type="button" @click="clearSearch" class="theme-form-back-link px-4 py-2.5">Clear</button>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="theme-table-header">
                            <th class="theme-table-header-cell">Transfer Ref</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="item in details.data" :key="item.id" class="theme-table-row group">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="theme-table-actions">
                                    <Link :href="route('stock-transfer-details.show', item.id)" class="theme-table-action-btn" title="View">
                                    ...
                                    <Link :href="route('stock-transfer-details.edit', item.id)" class="theme-table-action-btn theme-table-action-edit" title="Edit">
                        ...
                        <tr v-if="details.data.length === 0">
                            <td colspan="6" class="px-6 py-12 text-center text-slate-400 font-medium">No transfer items found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
