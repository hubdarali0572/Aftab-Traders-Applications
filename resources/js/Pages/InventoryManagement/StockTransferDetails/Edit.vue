...
<template>
    <AuthenticatedLayout>
        <Head title="Edit Transfer Item" />
        <div class="max-w-8xl mx-auto mb-5 flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-black text-slate-900">Edit Transfer Item</h2>
                <p class="text-sm text-slate-500 font-medium">Record ID: #{{ detail.id }}</p>
            </div>
            <Link :href="route('stock-transfer-details.index')" class="theme-form-back-link">Back to List</Link>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div class="flex justify-center pt-4">
                <PrimaryButton :disabled="form.processing" class="theme-btn-primary px-12 py-4">Update Transfer Item</PrimaryButton>
            </div>
        </form>
    </AuthenticatedLayout>
</template>
