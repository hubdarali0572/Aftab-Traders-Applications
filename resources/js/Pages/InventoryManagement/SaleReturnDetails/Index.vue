...
const props = defineProps({ details: Object, saleReturns: Array, filters: Object });

const searchQuery = ref(props.filters?.search ?? '');
const saleReturnId = ref(props.filters?.sale_return_id ?? '');
const isModalOpen = ref(false);
const selectedId = ref(null);

const openDeleteModal = (id) => { selectedId.value = id; isModalOpen.value = true; };
const confirmDelete = () => {
    router.delete(route('sale-return-details.destroy', selectedId.value), { onSuccess: () => { isModalOpen.value = false; } });
};

const applySearch = () => {
    router.get(route('sale-return-details.index'), {
        search: searchQuery.value || undefined,
        sale_return_id: saleReturnId.value || undefined,
    }, { preserveState: true, replace: true });
};

const clearSearch = () => { searchQuery.value = ''; saleReturnId.value = ''; applySearch(); };
</script>
...
<template>
    <AuthenticatedLayout>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="item in details.data" :key="item.id" class="theme-table-row group">
                            <td class="px-6 py-4 font-bold text-indigo-600">{{ item.sale_return?.reference_no }}</td>
                            <td class="px-6 py-4 text-sm font-bold text-slate-700 dark:text-slate-300">{{ item.sale_return?.sale?.invoice_no }}</td>
                            ...
                            <td class="px-6 py-4 text-right text-sm text-slate-600 dark:text-slate-400">${{ item.discount }}</td>
                            <td class="px-6 py-4 text-right text-sm text-slate-600 dark:text-slate-400">${{ item.tax }}</td>
        ...
        <ConfirmModal :show="isModalOpen" title="Remove Return Item" message="Remove this item? Stock and customer credit impact will be reversed." @close="isModalOpen = false" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
