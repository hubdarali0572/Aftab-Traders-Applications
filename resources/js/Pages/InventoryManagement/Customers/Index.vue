...
const props = defineProps({
    ...
    pageSubtitle: { type: String, default: 'Manage customer accounts and contact information.' },
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">{{ pageTitle }}</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">{{ pageSubtitle }}</p>
            </div>
            <Link :href="createHref()" class="theme-btn-primary">
        ...
        <div class="theme-table-card">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                <form @submit.prevent="applySearch" class="flex flex-col md:flex-row gap-3">
                    <input v-model="searchQuery" type="text" class="theme-form-input flex-1" placeholder="Search by code, name, company or phone..." />
                    <select v-if="!lockedType" v-model="customerType" class="theme-form-input md:w-48">
                        <option value="">{{ $t('All Types') }}</option>
                        <option v-for="t in customerTypes" :key="t" :value="t">{{ formatType(t) }}</option>
                    </select>
                    <div v-else class="theme-form-input md:w-48 flex items-center font-bold uppercase text-indigo-600 text-xs tracking-widest">
                        {{ formatType(lockedType) }}
                    </div>
                    <div class="flex gap-2">
                        <button type="submit" class="theme-btn-primary px-6 py-2.5">{{ $t('Search') }}</button>
                        <button v-if="filters?.search || (!lockedType && filters?.customer_type)" type="button" @click="clearSearch" class="theme-form-back-link px-4 py-2.5">{{ $t('Clear') }}</button>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="c in customers.data" :key="c.id" class="theme-table-row group">
                            ...
                            <td class="px-6 py-3 text-sm text-right font-bold text-slate-700 dark:text-slate-300">
                                {{ c.opening_balance_type === 'credit' ? 'Cr' : 'Dr' }} {{ c.opening_balance }}
                            </td>
                            <td class="px-6 py-3">
                            ...
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="theme-table-actions">
                                    ...
                                    <button @click="openDeleteModal(c.id)" class="theme-table-action-btn theme-table-action-delete" :title="$t('Delete')">
                        ...
                        <tr v-if="customers.data.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">{{ $t('No customers found.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <ConfirmModal
            ...
            message="Are you sure you want to permanently remove this customer? This action cannot be undone."
            @close="closeModal"
