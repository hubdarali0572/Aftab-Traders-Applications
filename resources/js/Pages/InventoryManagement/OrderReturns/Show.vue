    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center print:mb-4">
            ...
            <div class="flex flex-wrap gap-3 print:hidden">
                ...
                <Link :href="route('order-returns.edit', orderReturn.id)" class="theme-btn-primary px-6 py-2 rounded-full">Edit</Link>
                <Link :href="route('order-return-details.index', { order_return_id: orderReturn.id })" class="theme-btn-primary px-6 py-2 rounded-full bg-emerald-600 hover:bg-emerald-700">Line Items</Link>
                <button
                    v-if="canConvert()"
                    type="button"
                ...
                <Link :href="route('order-returns.index')" class="theme-form-back-link">Back</Link>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Order #</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ orderReturn.order?.order_no }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Customer</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ orderReturn.customer?.customer_name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Warehouse</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ orderReturn.warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Return Date</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ orderReturn.return_date }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Return Status</dt><dd class="font-bold capitalize">{{ orderReturn.return_status }}</dd></div>
                    <div v-if="orderReturn.converted_sale_return_id">
                        <dt class="text-xs font-bold text-slate-400 uppercase">Converted Sales Return</dt>
                        <dd class="font-bold">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            <tr v-if="!orderReturn.details?.length">
                                <td colspan="5" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">No return line items found.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
