...
const confirmDelete = () => {
    if (selectedId.value) {
        router.delete(route('order-returns.destroy', selectedId.value), {
...
const applySearch = () => {
    router.get(route('order-returns.index'), { search: searchQuery.value || undefined }, { preserveState: true, replace: true });
};
...
<template>
    <AuthenticatedLayout>
        <Head :title="$t('Order Returns')" />

        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">{{ $t('Order Returns') }}</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">{{ $t('Manage returns against customer orders.') }}</p>
            </div>
            <Link :href="route('order-returns.create')" class="theme-btn-primary">
        ...
        <div class="theme-table-card">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                <form @submit.prevent="applySearch" class="flex flex-col sm:flex-row gap-3">
                    <input v-model="searchQuery" type="text" class="theme-form-input flex-1" placeholder="Search by return ref, order #, or customer..." />
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        ...
                        <tr v-if="returns.data.length === 0">
                            <td colspan="9" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">{{ $t('No order return records found.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="theme-table-footer flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="text-[11px] font-bold text-indigo-700 uppercase tracking-widest">Showing {{ returns.from || 0 }} to {{ returns.to || 0 }} of {{ returns.total }} entries</div>
        ...
        <ConfirmModal :show="isModalOpen" title="Delete Order Return" message="Are you sure? This will remove the return and all its line items." @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
