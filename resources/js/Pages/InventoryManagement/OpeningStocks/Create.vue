...
const form = useForm({
    ...
    remarks: '',
    status: true,
    items: [{ product_id: '', quantity: '', unit_cost: 0 }],
});
...
<template>
    <AuthenticatedLayout>
        <Head :title="$t('Create Opening Stock')" />
        <div class="max-w-8xl mx-auto mb-5 flex justify-between items-center">
            <h2 class="text-2xl font-extrabold text-slate-900 dark:text-white">{{ $t('Create Opening Stock') }}</h2>
        ...
        <form @submit.prevent="submit" class="theme-form-card p-8 md:p-10 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <TextInput type="date" v-model="form.opening_date" class="w-full theme-form-input" required />
            ...
            <InventoryItemsEditor v-model="form.items" :products="products" :errors="form.errors" />

            <div class="flex justify-center"><PrimaryButton :disabled="form.processing">{{ $t('Save Opening Stock') }}</PrimaryButton></div>
        </form>
    </AuthenticatedLayout>
</template>
