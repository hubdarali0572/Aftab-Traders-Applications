...
const confirmDelete = () => {
    if (selectedId.value) {
        router.delete(route('sale-returns.destroy', selectedId.value), {
...
<template>
    <AuthenticatedLayout>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="r in returns.data" :key="r.id" class="theme-table-row group">
                            ...
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ r.customer?.customer_name || 'Walk-in' }}</td>
        ...
        <ConfirmModal :show="isModalOpen" title="Delete Sales Return" message="Are you sure? Stock and customer ledger effects will be reversed." @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
