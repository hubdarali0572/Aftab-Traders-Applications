    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('sale-returns.edit', saleReturn.id)" class="theme-btn-primary px-6 py-2 rounded-full">{{ $t('Edit') }}</Link>
                <Link :href="route('sale-returns.index')" class="theme-form-back-link">{{ $t('Back') }}</Link>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Invoice') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ saleReturn.sale?.invoice_no }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Customer') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ saleReturn.customer?.customer_name || 'Walk-in' }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Warehouse') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ saleReturn.warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Return Date') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ saleReturn.return_date }}</dd></div>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            <tr v-if="saleReturn.details.length === 0">
                                <td colspan="7" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">{{ $t('No return line items found.') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
