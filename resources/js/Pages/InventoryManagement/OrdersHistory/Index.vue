...
const filterParams = (extra = {}) => ({
    ...
    orders_sort: props.filters?.orders_sort,
    orders_direction: props.filters?.orders_direction,
...
const applyFilters = () => {
    router.get(route('orders-history.index'), filterParams(), { preserveState: true, replace: true });
};
...
const overviewStats = computed(() => [
    { title: 'Total Orders', value: props.summary?.total_orders ?? 0, tone: 'text-indigo-700 dark:text-indigo-300', bg: 'bg-indigo-50 dark:bg-indigo-500/10', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
    { title: 'Total Order Amount', value: money(props.summary?.total_order_amount), tone: 'text-emerald-700 dark:text-emerald-300', bg: 'bg-emerald-50 dark:bg-emerald-500/10', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    { title: 'Total Order Returns', value: props.summary?.total_order_returns ?? 0, tone: 'text-violet-700 dark:text-violet-300', bg: 'bg-violet-50 dark:bg-violet-500/10', icon: 'M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6' },
    ...
    { title: 'Net Order Amount', value: money(props.summary?.net_order_amount), tone: 'text-indigo-800 dark:text-indigo-200', bg: 'bg-indigo-100 dark:bg-indigo-500/15', icon: 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6', highlight: true },
    { title: 'Total Products Ordered', value: props.summary?.products_ordered ?? 0, tone: 'text-teal-700 dark:text-teal-300', bg: 'bg-teal-50 dark:bg-teal-500/10', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
...
const filterOrderRows = (rows) => {
    const q = ordersSearch.value.trim().toLowerCase();
    if (!q) return rows;
    return rows.filter((row) =>
        [row.order_no, row.customer_name, row.warehouse_name, row.products_label, row.processed_by]
...
const filterReturnRows = (rows) => {
    ...
    if (!q) return rows;
    return rows.filter((row) =>
        [row.reference_no, row.order_no, row.customer_name, row.warehouse_name, row.products_label, row.processed_by, row.return_reason]
            .some((v) => String(v || '').toLowerCase().includes(q))
    );
};

const filteredOrders = computed(() => filterOrderRows(props.orders?.data ?? []));
const filteredReturns = computed(() => filterReturnRows(props.returns?.data ?? []));

const sortOrders = (column) => {
    const current = props.filters?.orders_sort;
    const dir = props.filters?.orders_direction === 'asc' ? 'desc' : 'asc';
    router.get(route('orders-history.index'), filterParams({
        orders_sort: column,
        orders_direction: current === column ? dir : 'desc',
...
const exportUrl = (type) => {
    ...
    return `${route('orders-history.index')}?${params.toString()}`;
};
...
<template>
    <AuthenticatedLayout>
        <Head :title="$t('Order History')" />

        <div class="mb-6 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 print:hidden">
            <div>
                <p class="text-[11px] font-black uppercase tracking-widest text-indigo-500 mb-1">{{ $t('Order Management') }}</p>
                <h2 class="text-2xl font-black text-slate-800 tracking-tight dark:text-slate-100">{{ $t('Order History') }}</h2>
                <p class="text-sm text-slate-500 mt-1 dark:text-slate-400">{{ $t('Complete visibility into every order and return — customer, warehouse, products, and user.') }}</p>
            </div>
            <div class="flex flex-wrap gap-2 shrink-0">
                <a :href="exportUrl('orders')" class="theme-form-back-link px-4 py-2 text-sm font-bold">{{ $t('Export Orders') }}</a>
        ...
        <div class="theme-table-card mb-6 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-indigo-50 to-white dark:from-indigo-950/30 dark:to-slate-800">
                <h3 class="text-sm font-black uppercase tracking-widest text-slate-600 dark:text-slate-300">{{ $t('Order History Dashboard') }}</h3>
                <p class="text-xs text-slate-400 mt-0.5">{{ hasFilters ? $t('Metrics update instantly with your filters.') : $t('Overview of all order and return activity.') }}</p>
            </div>
        ...
        <div class="theme-table-card mb-8">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h3 class="text-sm font-black uppercase tracking-widest text-slate-500">{{ $t('Order History') }}</h3>
                    <p class="text-xs text-slate-400 mt-0.5">{{ $t('Warehouse, products, quantities, customer, and user for every order.') }}</p>
                </div>
                <div class="flex items-center gap-3 print:hidden">
                    <input v-model="ordersSearch" type="text" class="theme-form-input text-sm py-2 w-full sm:w-56" :placeholder="$t('Search table...')" />
                    <span class="text-xs font-bold text-indigo-600 whitespace-nowrap">{{ orders?.total ?? 0 }} {{ $t('records') }}</span>
                </div>
            </div>
                ...
                <table class="w-full text-left text-sm min-w-[1150px]">
                    <thead>
                        <tr class="theme-table-header">
                            <th class="theme-table-header-cell cursor-pointer" @click="sortOrders('order_date')">{{ $t('Order Date & Time') }}</th>
                            <th class="theme-table-header-cell cursor-pointer" @click="sortOrders('order_no')">{{ $t('Order No.') }}</th>
                            ...
                            <th class="theme-table-header-cell text-right cursor-pointer" @click="sortOrders('grand_total')">{{ $t('Grand Total') }}</th>
                            <th class="theme-table-header-cell text-right">{{ $t('Paid Amount') }}</th>
                            <th class="theme-table-header-cell text-right">{{ $t('Due Amount') }}</th>
                            <th class="theme-table-header-cell">{{ $t('Processed By') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="row in filteredOrders" :key="row.id" class="theme-table-row hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-4 py-3 text-slate-600 whitespace-nowrap">{{ row.datetime }}</td>
                            <td class="px-4 py-3 font-bold text-indigo-600">{{ row.order_no }}</td>
                            ...
                            <td class="px-4 py-3 text-slate-700 dark:text-slate-300">{{ row.processed_by }}</td>
                            <td class="px-4 py-3">
                                <div class="flex flex-col gap-1 items-start">
                                    <span class="inline-flex px-2 py-0.5 rounded-full text-[10px] font-black uppercase" :class="orderStatusClass(row.order_status)">{{ row.order_status }}</span>
                            ...
                            <td class="px-4 py-3 text-right whitespace-nowrap print:hidden">
                                <Link :href="route('orders.show', row.id)" class="text-xs font-bold text-indigo-600 hover:underline mr-3">{{ $t('View') }}</Link>
                                <Link :href="route('orders.show', row.id) + '?print=1'" class="text-xs font-bold text-slate-500 hover:text-slate-700">{{ $t('Print') }}</Link>
                            </td>
                        </tr>
                        <tr v-if="!filteredOrders.length"><td colspan="12" class="px-6 py-12 text-center text-slate-400">{{ $t('No order records found.') }}</td></tr>
                    </tbody>
                </table>
            </div>
            <div class="theme-table-footer flex flex-col sm:flex-row sm:items-center sm:justify-between print:hidden">
                <div class="text-[11px] font-bold text-indigo-700 uppercase tracking-widest dark:text-slate-200">
                    {{ $t('Showing') }} {{ orders.from || 0 }} {{ $t('to') }} {{ orders.to || 0 }} {{ $t('of') }} {{ orders.total }} {{ $t('entries') }}
                </div>
        ...
        <div class="theme-table-card mb-8">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h3 class="text-sm font-black uppercase tracking-widest text-slate-500">{{ $t('Order Return History') }}</h3>
                ...
                <table class="w-full text-left text-sm min-w-[1100px]">
                    <thead>
                        <tr class="theme-table-header">
                            ...
                            <th class="theme-table-header-cell">{{ $t('Original Order No.') }}</th>
                            <th class="theme-table-header-cell">{{ $t('Customer') }}</th>
                            <th class="theme-table-header-cell">{{ $t('Warehouse') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="row in filteredReturns" :key="row.id" class="theme-table-row hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <Link v-if="row.order_id" :href="route('orders.show', row.order_id)" class="text-indigo-600 font-bold hover:underline">{{ row.order_no }}</Link>
                            ...
                            <td class="px-4 py-3 text-right whitespace-nowrap print:hidden">
                                <Link :href="route('order-returns.show', row.id)" class="text-xs font-bold text-violet-600 hover:underline mr-3">{{ $t('View') }}</Link>
                                <Link :href="route('order-returns.show', row.id) + '?print=1'" class="text-xs font-bold text-slate-500 hover:text-slate-700">{{ $t('Print') }}</Link>
                            </td>
                        </tr>
