    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('purchase-return-details.edit', detail.id)" class="theme-btn-primary px-6 py-2">Edit</Link>
                <Link :href="route('purchase-return-details.index')" class="theme-form-back-link">Back</Link>
            </div>
        </div>

        <div class="theme-form-card p-10 mb-6">
            <h3 class="text-xs font-black uppercase text-slate-400 mb-6 tracking-widest">Return & Product</h3>
            <dl class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div><dt class="text-xs font-bold text-slate-400 uppercase">Return Ref</dt><dd class="font-bold text-indigo-600">{{ detail.purchase_return?.reference_no }}</dd></div>
        ...
        <div class="theme-form-card p-10 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="text-center p-6 bg-emerald-50 rounded-2xl">
                    <dt class="text-[10px] font-black uppercase text-emerald-400">Total Price</dt>
        ...
        <div class="theme-form-card p-10" v-if="detail.reason || detail.remarks">
            <h3 class="text-xs font-black uppercase text-slate-400 mb-6 tracking-widest">Notes</h3>
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div v-if="detail.reason"><dt class="text-xs font-bold text-slate-400 uppercase">Reason</dt><dd class="font-bold">{{ detail.reason }}</dd></div>
