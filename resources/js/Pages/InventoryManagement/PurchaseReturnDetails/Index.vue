...
const props = defineProps({ details: Object, purchaseReturns: Array, filters: Object });

const searchQuery = ref(props.filters?.search ?? '');
const returnId = ref(props.filters?.purchase_return_id ?? '');
const isModalOpen = ref(false);
const selectedId = ref(null);

const openDeleteModal = (id) => { selectedId.value = id; isModalOpen.value = true; };
const confirmDelete = () => {
    router.delete(route('purchase-return-details.destroy', selectedId.value), { onSuccess: () => { isModalOpen.value = false; } });
};

const applySearch = () => {
    router.get(route('purchase-return-details.index'), {
        search: searchQuery.value || undefined,
        purchase_return_id: returnId.value || undefined,
    }, { preserveState: true, replace: true });
};

const clearSearch = () => { searchQuery.value = ''; returnId.value = ''; applySearch(); };
</script>

<template>
    <AuthenticatedLayout>
        <Head :title="$t('Return Line Items')" />

        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">{{ $t('Purchase Return Details') }}</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">{{ $t('Manage products on purchase returns.') }}</p>
            </div>
            <Link :href="route('purchase-return-details.create')" class="theme-btn-primary">
        ...
        <div class="theme-table-card">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50">
                <form @submit.prevent="applySearch" class="flex flex-col md:flex-row gap-4">
                    <input v-model="searchQuery" type="text" class="theme-form-input flex-1" placeholder="Search by product or reference..." />
                    <select v-model="returnId" class="theme-form-input w-full md:w-64">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button v-if="filters?.search || filters?.purchase_return_id" type="button" @click="clearSearch" class="theme-form-back-link px-4 py-2.5">{{ $t('Clear') }}</button>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="item in details.data" :key="item.id" class="theme-table-row group">
                            <td class="px-6 py-4 font-bold text-indigo-600">{{ item.purchase_return?.reference_no }}</td>
                        ...
                        <tr v-if="details.data.length === 0">
                            <td colspan="6" class="px-6 py-12 text-center text-slate-400 font-medium">{{ $t('No return line items found.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <ConfirmModal :show="isModalOpen" title="Remove Return Item" message="Remove this item? Stock will be reversed." @close="isModalOpen = false" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
