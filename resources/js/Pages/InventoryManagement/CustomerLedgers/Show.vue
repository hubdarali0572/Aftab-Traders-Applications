    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link v-if="ledger.reference_type === 'Manual'" :href="route('customer-ledgers.edit', ledger.id)" class="theme-btn-primary px-6 py-2">{{ $t('Edit') }}</Link>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Customer') }}</dt><dd class="font-bold">{{ ledger.customer?.customer_name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Date') }}</dt><dd class="font-bold text-slate-700">{{ ledger.transaction_date }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Reference') }}</dt><dd class="font-bold">{{ ledger.reference_no || '—' }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Source') }}</dt><dd class="font-bold uppercase text-indigo-600">{{ ledger.reference_type }}</dd></div>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="theme-form-card p-10 bg-rose-50 dark:bg-rose-900/10 text-center border-rose-100">
                    <dt class="text-xs font-black uppercase text-rose-400 mb-2">{{ $t('Debit') }}</dt>
                ...
                <div class="theme-form-card p-10 bg-emerald-50 dark:bg-emerald-900/10 text-center border-emerald-100">
                    <dt class="text-xs font-black uppercase text-emerald-400 mb-2">{{ $t('Credit') }}</dt>
                ...
                <div class="theme-form-card p-10 bg-indigo-50 dark:bg-indigo-900/10 text-center border-indigo-100">
                    <dt class="text-xs font-black uppercase text-indigo-400 mb-2">{{ $t('Running Balance') }}</dt>
