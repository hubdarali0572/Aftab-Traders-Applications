...
<template>
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-5 flex justify-between items-center">
            <h2 class="text-2xl font-black text-slate-800">{{ $t('Edit Purchase') }}</h2>
            <Link :href="route('purchases.index')" class="theme-form-back-link font-bold">{{ $t('Back to List') }}</Link>
        </div>
        ...
        <form @submit.prevent="submit" class="theme-form-card p-8 md:p-10 space-y-6">
            ...
            <div class="flex justify-center pt-4">
                <PrimaryButton :disabled="form.processing">{{ $t('Update Purchase') }}</PrimaryButton>
            </div>
        </form>
    </AuthenticatedLayout>
</template>
