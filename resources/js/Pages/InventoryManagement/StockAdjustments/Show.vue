        ...
        <div class="space-y-6">
            ...
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Warehouse') }}</dt><dd class="font-bold">{{ adjustment.warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Adjustment Date') }}</dt><dd class="font-bold text-slate-700">{{ adjustment.adjustment_date }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Type') }}</dt><dd class="font-bold uppercase text-indigo-600">{{ adjustment.adjustment_type }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Created By') }}</dt><dd class="font-bold text-slate-700">{{ adjustment.user?.name }}</dd></div>
                </div>
            </div>
