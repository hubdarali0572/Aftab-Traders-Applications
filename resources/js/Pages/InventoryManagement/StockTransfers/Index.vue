...
const confirmDelete = () => {
    if (selectedId.value) {
        router.delete(route('stock-transfers.destroy', selectedId.value), {
...
const applySearch = () => {
    router.get(route('stock-transfers.index'), { search: searchQuery.value || undefined }, {
        preserveState: true,
        replace: true,
    });
};
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">{{ $t('Stock Transfers') }}</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">{{ $t('Move inventory between warehouses.') }}</p>
            </div>
            <Link :href="route('stock-transfers.create')" class="theme-btn-primary">
        ...
        <div class="theme-table-card">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                <form @submit.prevent="applySearch" class="flex flex-col sm:flex-row gap-3">
                    <input v-model="searchQuery" type="text" class="theme-form-input flex-1" :placeholder="$t('Search by reference, product, warehouse, or user...')" />
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="theme-table-header">
                            ...
                            <th class="theme-table-header-cell">{{ $t('Date & Time') }}</th>
                            <th class="theme-table-header-cell">{{ $t('Product') }}</th>
                            <th class="theme-table-header-cell">{{ $t('From') }}</th>
                            <th class="theme-table-header-cell">{{ $t('To') }}</th>
                            <th class="theme-table-header-cell text-right">{{ $t('Quantity') }}</th>
                            <th class="theme-table-header-cell">{{ $t('Performed By') }}</th>
                            <th class="theme-table-header-cell">{{ $t('Status') }}</th>
                            <th class="theme-table-header-cell text-right">{{ $t('Actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="transfer in transfers.data" :key="transfer.id" class="theme-table-row group">
                            <td class="px-6 py-3 font-bold text-slate-800 dark:text-slate-200">{{ transfer.reference_no }}</td>
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ formatDateTime(transfer) }}</td>
                            <td class="px-6 py-3 text-sm font-semibold text-slate-700 dark:text-slate-300">{{ transfer.product?.name }}</td>
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ transfer.from_warehouse?.name }}</td>
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ transfer.to_warehouse?.name }}</td>
                            <td class="px-6 py-3 text-sm text-right font-bold text-slate-700 dark:text-slate-300">{{ transfer.quantity }}</td>
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ transfer.user?.name ?? '—' }}</td>
                            <td class="px-6 py-3">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase"
                                    :class="transfer.status ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400'"
                                >
                            ...
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="theme-table-actions">
                                    <Link :href="route('stock-transfers.show', transfer.id)" class="theme-table-action-btn" :title="$t('View')">
                                    ...
                                    <Link :href="route('stock-transfers.edit', transfer.id)" class="theme-table-action-btn theme-table-action-edit" :title="$t('Edit')">
                                    ...
                                    <button @click="openDeleteModal(transfer.id)" class="theme-table-action-btn theme-table-action-delete" :title="$t('Delete')">
                        ...
                        <tr v-if="transfers.data.length === 0">
                            <td colspan="9" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">{{ $t('No stock transfer records found.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="theme-table-footer flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="text-[11px] font-bold text-indigo-700 uppercase tracking-widest text-center sm:text-left dark:text-slate-200">
                    {{ $t('Showing') }} {{ transfers.from || 0 }} {{ $t('to') }} {{ transfers.to || 0 }} {{ $t('of') }} {{ transfers.total }} {{ $t('entries') }}
                </div>
        ...
        <ConfirmModal
            :show="isModalOpen"
            :title="$t('Delete Stock Transfer')"
            :message="$t('Are you sure you want to permanently remove this transfer? Stock will be reversed in both warehouses.')"
            @close="closeModal"
