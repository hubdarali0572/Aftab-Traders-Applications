...
const statusClass = (status) => {
    const map = {
        ...
        in_transit: 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400',
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="theme-table-card">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="theme-table-header">
                            ...
                            <th class="theme-table-header-cell">{{ $t('Stock Status') }}</th>
                            <th class="theme-table-header-cell text-right">{{ $t('Actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="t in transfers.data" :key="t.id" class="theme-table-row group">
                            <td class="px-6 py-3 font-bold text-slate-800 dark:text-slate-200">{{ t.reference_no }}</td>
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ t.transfer_date }}</td>
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ t.from_warehouse?.name }}</td>
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ t.to_warehouse?.name }}</td>
                            <td class="px-6 py-3 text-sm text-right font-bold text-slate-700 dark:text-slate-300">{{ t.total_quantity }}</td>
                            <td class="px-6 py-3 text-sm text-right font-bold text-slate-700 dark:text-slate-300">${{ t.total_amount }}</td>
                            <td class="px-6 py-3">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase" :class="statusClass(t.stock_status)">
                                    {{ statusLabel(t.stock_status) }}
                                </span>
                            </td>
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="theme-table-actions">
                                    <Link :href="route('stock-transfers.show', t.id)" class="theme-table-action-btn" :title="$t('View')">
                                    ...
                                    <Link :href="route('stock-transfers.edit', t.id)" class="theme-table-action-btn theme-table-action-edit" :title="$t('Edit')">
                                    ...
                                    <button @click="openDeleteModal(t.id)" class="theme-table-action-btn theme-table-action-delete" :title="$t('Delete')">
                        ...
                        <tr v-if="transfers.data.length === 0">
                            <td colspan="8" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">{{ $t('No stock transfer records found.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="theme-table-footer flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="text-[11px] font-bold text-indigo-700 uppercase tracking-widest text-center sm:text-left">
                    Showing {{ transfers.from || 0 }} to {{ transfers.to || 0 }} of {{ transfers.total }} entries
                </div>
        ...
        <ConfirmModal :show="isModalOpen" title="Delete Stock Transfer" message="Are you sure you want to permanently remove this transfer? All line items will be reversed from inventory." @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
