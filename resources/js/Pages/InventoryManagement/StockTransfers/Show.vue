    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('stock-transfers.edit', transfer.id)" class="theme-btn-primary px-6 py-2 rounded-full">Edit</Link>
                <Link :href="route('stock-transfers.index')" class="theme-form-back-link">Back</Link>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">From Warehouse</dt><dd class="font-bold">{{ transfer.from_warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">To Warehouse</dt><dd class="font-bold">{{ transfer.to_warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Transfer Date</dt><dd class="font-bold text-slate-700">{{ transfer.transfer_date }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Created By</dt><dd class="font-bold text-slate-700">{{ transfer.user?.name }}</dd></div>
                </div>
            </div>
            ...
            <div class="theme-form-card" v-if="transfer.details?.length">
                <div class="p-8 md:p-10">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Line Items</h3>
                        <Link :href="route('stock-transfer-details.create')" class="text-xs font-bold text-indigo-600 hover:text-indigo-800">+ Add Item</Link>
                    </div>
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="theme-table-header">
                                    ...
                                    <th class="theme-table-header-cell text-right">Unit Cost</th>
