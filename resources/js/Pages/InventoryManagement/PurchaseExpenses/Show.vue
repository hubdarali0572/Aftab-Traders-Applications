    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('purchase-expenses.edit', expense.id)" class="theme-btn-primary px-6 py-2">Edit</Link>
                <Link :href="route('purchase-expenses.index')" class="theme-form-back-link">Back</Link>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Purchase</dt><dd class="font-bold text-indigo-600">{{ expense.purchase?.purchase_no }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Expense Type</dt><dd class="font-bold text-slate-700">{{ expense.expense_type }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">Recorded By</dt><dd class="font-bold text-slate-700">{{ expense.user?.name || '—' }}</dd></div>
                </div>
            </div>
