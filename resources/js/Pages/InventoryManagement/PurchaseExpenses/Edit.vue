...
<template>
    <AuthenticatedLayout>
        <Head :title="$t('Edit Purchase Expense')" />
        <div class="max-w-8xl mx-auto mb-5 flex justify-between items-center">
            <h2 class="text-2xl font-black text-slate-900">{{ $t('Edit Purchase Expense') }}</h2>
            <Link :href="route('purchase-expenses.index')" class="theme-form-back-link">{{ $t('Back to List') }}</Link>
        </div>
        ...
        <form @submit.prevent="submit" class="theme-form-card p-8 md:p-10 space-y-6">
            ...
            <div class="flex justify-center pt-4">
                <PrimaryButton :disabled="form.processing">{{ $t('Update Expense') }}</PrimaryButton>
            </div>
        </form>
    </AuthenticatedLayout>
</template>
