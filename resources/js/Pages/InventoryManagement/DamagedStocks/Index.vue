...
<template>
    <AuthenticatedLayout>
        <Head title="Damaged Stocks" />

        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">Damaged Stocks</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">Record and track damaged inventory write-offs.</p>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="stock in stocks.data" :key="stock.id" class="theme-table-row group">
                            ...
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="theme-table-actions">
                                    <Link :href="route('damaged-stocks.show', stock.id)" class="theme-table-action-btn" title="View">
                                    ...
                                    <Link :href="route('damaged-stocks.edit', stock.id)" class="theme-table-action-btn theme-table-action-edit" title="Edit">
                                    ...
                                    <button @click="openDeleteModal(stock.id)" class="theme-table-action-btn theme-table-action-delete" title="Delete">
                        ...
                        <tr v-if="stocks.data.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">No damaged stock records found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
