...
const confirmDelete = () => {
    if (selectedId.value) {
        router.delete(route('damaged-stocks.destroy', selectedId.value), {
...
const applySearch = () => {
    router.get(route('damaged-stocks.index'), { search: searchQuery.value || undefined }, {
        preserveState: true,
        replace: true,
    });
};
...
<template>
    <AuthenticatedLayout>
        <Head :title="$t('Damaged Stocks')" />

        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">{{ $t('Damaged Stocks') }}</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">{{ $t('Record and track damaged inventory write-offs.') }}</p>
            </div>
            <Link :href="route('damaged-stocks.create')" class="theme-btn-primary">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="stock in stocks.data" :key="stock.id" class="theme-table-row group">
                            ...
                            <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ stock.damage_date }}</td>
                            ...
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="theme-table-actions">
                                    <Link :href="route('damaged-stocks.show', stock.id)" class="theme-table-action-btn" :title="$t('View')">
                                    ...
                                    <Link :href="route('damaged-stocks.edit', stock.id)" class="theme-table-action-btn theme-table-action-edit" :title="$t('Edit')">
                                    ...
                                    <button @click="openDeleteModal(stock.id)" class="theme-table-action-btn theme-table-action-delete" :title="$t('Delete')">
                        ...
                        <tr v-if="stocks.data.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400 font-medium dark:text-slate-500">{{ $t('No damaged stock records found.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <ConfirmModal :show="isModalOpen" title="Delete Damaged Stock Record" message="Are you sure you want to permanently remove this damage record?" @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
