...
const props = defineProps({ details: Object, filters: Object, damaged_stocks: Array });

const searchQuery = ref(props.filters?.search ?? '');
const damagedStockId = ref(props.filters?.damaged_stock_id ?? '');
const isModalOpen = ref(false);
const selectedId = ref(null);

const openDeleteModal = (id) => { selectedId.value = id; isModalOpen.value = true; };
const confirmDelete = () => {
    router.delete(route('damaged-stock-details.destroy', selectedId.value), {
        onSuccess: () => { isModalOpen.value = false; },
    });
};

const applySearch = () => {
    router.get(route('damaged-stock-details.index'), {
        search: searchQuery.value || undefined,
        damaged_stock_id: damagedStockId.value || undefined,
    }, { preserveState: true, replace: true });
};

const clearSearch = () => { searchQuery.value = ''; damagedStockId.value = ''; applySearch(); };
</script>

<template>
    <AuthenticatedLayout>
        <Head title="Damaged Stock Items" />

        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-700 tracking-tight dark:text-slate-100">Damaged Stock Details</h2>
                <p class="text-sm text-slate-500 mt-1 font-medium dark:text-slate-400">Manage individual damaged product line items.</p>
            </div>
            <Link :href="route('damaged-stock-details.create')" class="theme-btn-primary">
            ...
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50">
                <form @submit.prevent="applySearch" class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="w-full md:w-64">
                        <select v-model="damagedStockId" class="theme-form-input w-full">
                            <option value="">All Damage Records</option>
                    ...
                    <div class="flex gap-2">
                        ...
                        <button v-if="filters?.search || filters?.damaged_stock_id" type="button" @click="clearSearch" class="theme-form-back-link px-4 py-2.5">Clear</button>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="theme-table-header">
                            <th class="theme-table-header-cell">Damage Ref</th>
                            <th class="theme-table-header-cell">Product</th>
                            <th class="theme-table-header-cell">Damage Reason</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="item in details.data" :key="item.id" class="theme-table-row group">
                            <td class="px-6 py-4 font-bold text-indigo-600 dark:text-indigo-400">{{ item.damaged_stock?.reference_no }}</td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ item.product?.name }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-rose-600 dark:text-rose-400 font-medium">{{ item.damage_reason }}</td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="theme-table-actions">
                                    <Link :href="route('damaged-stock-details.show', item.id)" class="theme-table-action-btn" title="View">
                                    ...
                                    <Link :href="route('damaged-stock-details.edit', item.id)" class="theme-table-action-btn theme-table-action-edit" title="Edit">
                                    ...
                                    <button @click="openDeleteModal(item.id)" class="theme-table-action-btn theme-table-action-delete" title="Delete">
                        ...
                        <tr v-if="details.data.length === 0">
                            <td colspan="6" class="px-6 py-12 text-center text-slate-400 font-medium">No damaged items found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <ConfirmModal :show="isModalOpen" title="Remove Damaged Item" message="Remove this item? Inventory will be reversed." @close="isModalOpen = false" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
