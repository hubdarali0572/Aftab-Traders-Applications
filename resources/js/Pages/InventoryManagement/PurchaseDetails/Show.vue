    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <Link :href="route('purchase-details.edit', detail.id)" class="theme-btn-primary px-6 py-2">{{ $t('Edit') }}</Link>
                <Link :href="route('purchase-details.index')" class="theme-form-back-link">{{ $t('Back') }}</Link>
            </div>
        </div>

        <div class="theme-form-card p-10 mb-6">
            <h3 class="text-xs font-black uppercase text-slate-400 mb-6 tracking-widest">{{ $t('Purchase & Product') }}</h3>
            <dl class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('PO #') }}</dt><dd class="font-bold text-indigo-600">{{ detail.purchase?.purchase_no }}</dd></div>
        ...
        <div class="theme-form-card p-10 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div class="text-center p-6 bg-slate-50 rounded-2xl">
                    <dt class="text-[10px] font-black uppercase text-slate-400">{{ $t('Free Qty') }}</dt>
                ...
                <div class="text-center p-6 bg-emerald-50 rounded-2xl">
                    <dt class="text-[10px] font-black uppercase text-emerald-400">{{ $t('Line Total') }}</dt>
        ...
        <div class="theme-form-card p-10" v-if="detail.batch_no || detail.serial_no || detail.remarks">
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div v-if="detail.manufacturing_date"><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Mfg Date') }}</dt><dd class="font-bold">{{ detail.manufacturing_date }}</dd></div>
                <div v-if="detail.expiry_date"><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Expiry') }}</dt><dd class="font-bold">{{ detail.expiry_date }}</dd></div>
