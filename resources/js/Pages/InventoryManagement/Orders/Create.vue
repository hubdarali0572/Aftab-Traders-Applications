...
<template>
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-5 flex justify-between items-center">
            <h2 class="text-2xl font-black text-slate-800 dark:text-slate-100">{{ $t('New Order') }}</h2>
            <Link :href="route('orders.index')" class="theme-form-back-link">{{ $t('Back to List') }}</Link>
        </div>
        ...
        <form @submit.prevent="submit" class="theme-form-card p-8 md:p-10 space-y-6">
            <div class="rounded-lg border border-indigo-100 bg-indigo-50/70 px-4 py-3 text-sm text-indigo-800 dark:border-indigo-500/30 dark:bg-indigo-500/10 dark:text-indigo-200">
                <p class="font-bold">{{ $t('How amounts work') }}</p>
                <p class="mt-1">
                    <strong>{{ $t('Subtotal') }}</strong> {{ $t('is calculated automatically from Order Details (products). It stays') }} <strong>$0.00</strong> {{ $t('until you add line items.') }}
                    <strong>{{ $t('converted_sale_id') }}</strong> {{ $t('stays empty until you use') }} <em>{{ $t('Convert to Sale') }}</em> {{ $t('on the order — it is a system link, not a form field.') }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500">{{ $t('Updates after you add products under Order Details.') }}</p>
                </div>
                <div>
            ...
            <div class="flex justify-center"><PrimaryButton :disabled="form.processing">{{ $t('Save Order & Add Items') }}</PrimaryButton></div>
        </form>
    </AuthenticatedLayout>
</template>
