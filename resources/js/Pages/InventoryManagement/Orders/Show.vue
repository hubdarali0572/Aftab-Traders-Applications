    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-8xl mx-auto mb-8 flex justify-between items-center print:mb-4">
            ...
            <div class="flex flex-wrap gap-3 print:hidden">
                ...
                <Link :href="route('orders.edit', order.id)" class="theme-btn-primary px-6 py-2 rounded-full">{{ $t('Edit') }}</Link>
                <Link :href="route('order-details.index', { order_id: order.id })" class="theme-btn-primary px-6 py-2 rounded-full bg-emerald-600 hover:bg-emerald-700">{{ $t('Line Items') }}</Link>
                <button
                    v-if="canConvert()"
                    type="button"
                    @click="convertToSale"
                ...
                <Link :href="route('orders.index')" class="theme-form-back-link">{{ $t('Back') }}</Link>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="theme-form-card p-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Customer') }}</dt><dd class="font-bold">{{ order.customer?.customer_name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Warehouse') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ order.warehouse?.name }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Order Date') }}</dt><dd class="font-bold text-slate-700 dark:text-slate-300">{{ order.order_date }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Order Type') }}</dt><dd class="font-bold capitalize">{{ order.order_type }}</dd></div>
                    <div><dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Created By') }}</dt><dd class="font-bold text-slate-700">{{ order.user?.name || '—' }}</dd></div>
                    <div v-if="order.converted_sale_id">
                        <dt class="text-xs font-bold text-slate-400 uppercase">{{ $t('Converted Sale') }}</dt>
                        <dd class="font-bold">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="theme-form-card p-8 text-center">
                    <dt class="text-xs font-black uppercase text-slate-400 mb-2">{{ $t('Tax + Other') }}</dt>
