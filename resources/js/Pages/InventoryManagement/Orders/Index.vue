...
const confirmDelete = () => {
    if (selectedId.value) {
        router.delete(route('orders.destroy', selectedId.value), {
...
<template>
    <AuthenticatedLayout>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="order in orders.data" :key="order.id" class="theme-table-row group">
                            <td class="px-6 py-3 font-bold text-indigo-600 dark:text-indigo-400">{{ order.order_no }}</td>
        ...
        <ConfirmModal
            :show="isModalOpen"
            title="Delete Order"
            message="Are you sure you want to permanently remove this order and all its line items?"
            @close="closeModal"
