...
const props = defineProps({ 
    details: Object, 
    filters: Object,
    adjustments: Array // Added to filter by adjustment specifically
});

const searchQuery = ref(props.filters?.search ?? '');
const adjustmentId = ref(props.filters?.adjustment_id ?? '');
...
const confirmDelete = () => {
    router.delete(route('stock-adjustment-details.destroy', selectedId.value), {
        onSuccess: () => isModalOpen.value = false
    });
};

const applySearch = () => {
    router.get(route('stock-adjustment-details.index'), { 
        search: searchQuery.value || undefined,
        adjustment_id: adjustmentId.value || undefined
    }, { 
        preserveState: true,
        replace: true 
    });
...
<template>
    <AuthenticatedLayout>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="item in details.data" :key="item.id" class="theme-table-row group">
                            <td class="px-6 py-4 font-bold text-indigo-600 dark:text-indigo-400">{{ item.stock_adjustment?.reference_no }}</td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ item.product?.name }}</div>
                                <div class="text-[10px] text-slate-400 font-medium italic">{{ item.reason || 'No reason specified' }}</div>
                            </td>
                            <td class="px-6 py-4 text-right text-slate-600 dark:text-slate-400">{{ item.system_quantity }}</td>
                            <td class="px-6 py-4 text-right text-slate-600 dark:text-slate-400">{{ item.physical_quantity }}</td>
                            <td class="px-6 py-4 text-right">
                                <span :class="item.adjustment_quantity >= 0 ? 'text-emerald-600' : 'text-rose-600'" class="font-bold">
        ...
        <ConfirmModal :show="isModalOpen" title="Remove Item" message="Remove this item from the adjustment list?" @close="isModalOpen = false" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>