...
const closeModal = () => {
    ...
    setTimeout(() => { selectedCategory.value = null; }, 300);
};

const confirmDelete = () => {
    if (selectedCategory.value) {
        router.delete(route('product-categories.destroy', selectedCategory.value.id), {
...
const applySearch = () => {
    router.get(route('product-categories.index'), { search: searchQuery.value || undefined }, {
        preserveState: true,
        replace: true,
    });
};
...
<template>
    <AuthenticatedLayout>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="category in categories.data" :key="category.id" class="theme-table-row group">
                            <td class="px-6 py-2"><div class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ category.code }}</div></td>
                            <td class="px-6 py-2"><div class="text-sm text-slate-600 dark:text-slate-300">{{ category.name }}</div></td>
                            <td class="px-6 py-2"><div class="text-sm text-slate-600 dark:text-slate-300">{{ category.category_type }}</div></td>
                            <td class="px-6 py-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold" :class="category.status ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'">
                                    {{ category.status ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-2 whitespace-nowrap text-right">
                                <div class="theme-table-actions">
                                    <Link :href="route('product-categories.edit', category.id)" class="theme-table-action-btn theme-table-action-edit" title="Edit Category">
                                    ...
                                    <button @click="openDeleteModal(category)" class="theme-table-action-btn theme-table-action-delete" title="Delete Category">
        ...
        <ConfirmModal
            ...
            message="Are you sure you want to permanently remove this product category from the system?"
            confirm-label="Yes, Delete Category"
            cancel-label="No, Keep Category"
            :badge="selectedCategory?.name"
            :badge-initial="selectedCategory?.name?.slice(0, 1)"
            @close="closeModal"
