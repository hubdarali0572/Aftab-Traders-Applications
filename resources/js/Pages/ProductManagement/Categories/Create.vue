...
<template>
    ...
    <AuthenticatedLayout>
        <div class="max-w-8xl mx-auto mb-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight dark:text-slate-100">{{ $t('Create New Product Category') }}</h2>
            </div>
        ...
        <div class="max-w-8xl mx-auto pb-5">
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="flex items-center justify-center pt-4">
                    <PrimaryButton class="theme-btn-primary px-12 py-4 rounded-full text-white font-black text-xs uppercase tracking-widest active:scale-95" :class="{ 'opacity-50 cursor-not-allowed': form.processing }" :disabled="form.processing">{{ $t('Create Category') }}</PrimaryButton>
                </div>
            </form>
        </div>
    </AuthenticatedLayout>
