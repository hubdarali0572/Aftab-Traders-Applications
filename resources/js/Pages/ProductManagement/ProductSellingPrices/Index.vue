...
const props = defineProps({ productSellingPrices: Object, filters: Object });
const page = usePage();
...
const openDeleteModal = (price) => { selectedPrice.value = price; isModalOpen.value = true; };
const closeModal = () => { isModalOpen.value = false; setTimeout(() => { selectedPrice.value = null; }, 300); };
const confirmDelete = () => {
    if (selectedPrice.value) {
        router.delete(route('product-selling-prices.destroy', selectedPrice.value.id), { onSuccess: () => closeModal(), onFinish: () => closeModal() });
    }
};

const applySearch = () => {
    router.get(route('product-selling-prices.index'), { search: searchQuery.value || undefined }, { preserveState: true, replace: true });
};
...
<template>
    <AuthenticatedLayout>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="price in productSellingPrices.data" :key="price.id" class="theme-table-row group">
                            <td class="px-6 py-2"><div class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ price.product?.name || '—' }}</div></td>
                            <td class="px-6 py-2"><div class="text-sm text-slate-600 dark:text-slate-300">{{ formatPrice(price.retail_price) }}</div></td>
                            <td class="px-6 py-2"><div class="text-sm text-slate-600 dark:text-slate-300">{{ formatPrice(price.wholesale_price) }}</div></td>
                            <td class="px-6 py-2"><div class="text-sm text-slate-600 dark:text-slate-300">{{ formatPrice(price.dealer_price) }}</div></td>
                            <td class="px-6 py-2"><div class="text-sm text-slate-600 dark:text-slate-300">{{ formatPrice(price.distributor_price) }}</div></td>
                            <td class="px-6 py-2"><div class="text-sm text-slate-600 dark:text-slate-300">{{ formatPrice(price.online_price) }}</div></td>
                          <td class="px-6 py-2">
                            ...
                            <td class="px-6 py-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold" :class="price.is_default ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'">{{ price.is_default ? 'Yes' : 'No' }}</span>
                            </td>
                            <td class="px-6 py-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold" :class="price.status ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'">{{ price.status ? 'Active' : 'Inactive' }}</span>
                            </td>
                            <td class="px-6 py-2 whitespace-nowrap text-right">
                                <div class="theme-table-actions">
                                    <!-- Show -->
                                    <Link :href="route('product-selling-prices.show', price.id)" class="theme-table-action-btn theme-table-action-view" title="Show Price">
                                        <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                            <path d="M2.036 12.322a1.012 1.012 0 010-.644C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" stroke-linecap="round" stroke-linejoin="round" />
                                            <path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </Link>

                                    <!-- Edit -->
                                    <Link :href="route('product-selling-prices.edit', price.id)" class="theme-table-action-btn theme-table-action-edit" title="Edit Price">
                                        <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                    ...
                                    <!-- Delete -->
                                    <button @click="openDeleteModal(price)" class="theme-table-action-btn theme-table-action-delete" title="Delete Price">
                                        <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <ConfirmModal :show="isModalOpen" title="Delete Selling Price" message="Are you sure you want to permanently remove this selling price from the system?" confirm-label="Yes, Delete Price" cancel-label="No, Keep Price" :badge="selectedPrice?.product?.name" :badge-initial="selectedPrice?.product?.name?.slice(0, 1)" @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
