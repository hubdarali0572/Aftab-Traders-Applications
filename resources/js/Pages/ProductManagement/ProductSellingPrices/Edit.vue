...
const form = useForm({
    product_id: props.productSellingPrice.product_id ?? '',

    purchase_price: props.productSellingPrice.purchase_price ?? 0,
    landing_cost: props.productSellingPrice.landing_cost ?? 0,
    cost_price: props.productSellingPrice.cost_price ?? 0,

    retail_price: props.productSellingPrice.retail_price ?? '',
    wholesale_price: props.productSellingPrice.wholesale_price ?? '',
    dealer_price: props.productSellingPrice.dealer_price ?? '',
    distributor_price: props.productSellingPrice.distributor_price ?? '',
    online_price: props.productSellingPrice.online_price ?? '',

    minimum_selling_price: props.productSellingPrice.minimum_selling_price ?? '',
    maximum_discount: props.productSellingPrice.maximum_discount ?? '',
    profit_margin: props.productSellingPrice.profit_margin ?? '',

    effective_from: props.productSellingPrice.effective_from ?? '',
    effective_to: props.productSellingPrice.effective_to ?? '',

    is_default: props.productSellingPrice.is_default ?? true,
    status: props.productSellingPrice.status ?? true,
});

const submit = () => {
    form.put(route('product-selling-prices.update', props.productSellingPrice.id));
};
</script>

<template>
    <Head :title="$t('Edit Selling Price')" />
    <AuthenticatedLayout>
        <div class="max-w-8xl mx-auto mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight dark:text-slate-100">{{ $t('Edit Selling Price') }}</h2>
            </div>
        ...
        <div class="max-w-8xl mx-auto pb-4">
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="flex items-center justify-center pt-4">
                    <PrimaryButton class="theme-btn-primary px-12 py-4 rounded-full text-white font-black text-xs uppercase tracking-widest active:scale-95" :class="{ 'opacity-50 cursor-not-allowed': form.processing }" :disabled="form.processing">Update Selling Price</PrimaryButton>
                </div>
            </form>
        </div>
    </AuthenticatedLayout>
