...
<template>
    ...
    <AuthenticatedLayout>
        <div class="max-w-8xl mx-auto mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight dark:text-slate-100">{{ $t('Edit Warehouse') }}</h2>
            </div>
        ...
        <div class="max-w-8xl mx-auto pb-5">
            <form @submit.prevent="submit" class="space-y-3">
                <div class="theme-form-card">
                    <div class="p-8 md:p-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-10 gap-y-8">
                            ...
                            <div class="flex flex-col">
                                <InputLabel for="email" :value="$t('Type')" class="theme-form-label ml-1" />
                                ...
                                >
                                    ...
                                    <option value="Retail Store">{{ $t('Retail Store') }}</option>
                                </select>
                ...
                <div class="flex items-center justify-center pt-4">
                    <PrimaryButton class="theme-btn-primary px-12 py-4 rounded-full text-white font-black text-xs uppercase tracking-widest active:scale-95" :class="{ 'opacity-50 cursor-not-allowed': form.processing }" :disabled="form.processing">{{ $t('Update Warehouse') }}</PrimaryButton>
                </div>
            </form>
        </div>
    </AuthenticatedLayout>
