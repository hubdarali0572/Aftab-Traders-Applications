...
<template>
    ...
    <AuthenticatedLayout>
        <div class="max-w-8xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight dark:text-slate-100">{{ $t('Edit Brand') }}</h2>
                <p class="text-sm text-slate-800 mt-1 font-medium dark:text-slate-400">{{ $t('Update existing brand details.') }}</p>
            </div>
            <Link :href="route('brands.index')" class="theme-form-back-link">
                ...
                <span class="text-slate-900">{{ $t('Back to Brand List') }}</span>
            </Link>
        </div>
                    ...
                    <div class="p-8 md:p-10">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                            <div class="flex flex-col">
                                ...
                                <TextInput id="name" type="text" class="theme-form-input" v-model="form.name" required autofocus placeholder="e.g. SparkLite" />
                            ...
                            <div class="flex flex-col md:col-span-2">
                                ...
                                <textarea id="description" class="theme-form-input min-h-[120px] resize-y" v-model="form.description" placeholder="Short description of the brand..."></textarea>
