...
const openDeleteModal = (product) => { selectedProduct.value = product; isModalOpen.value = true; };
const closeModal = () => { isModalOpen.value = false; setTimeout(() => { selectedProduct.value = null; }, 300); };
const confirmDelete = () => {
    if (selectedProduct.value) {
        router.delete(route('products.destroy', selectedProduct.value.id), { onSuccess: () => closeModal(), onFinish: () => closeModal() });
    }
};

const applySearch = () => {
    router.get(route('products.index'), { search: searchQuery.value || undefined }, { preserveState: true, replace: true });
};
...
<template>
    <AuthenticatedLayout>
                        ...
                        <tr v-for="product in products.data" :key="product.id" class="theme-table-row group">
                          ...
                          <td class="px-6 py-2 whitespace-nowrap text-right">
                            <div class="theme-table-actions">
                                <!-- Show Product (Updated Icon) -->
                                <Link :href="route('products.show', product.id)" class="theme-table-action-btn theme-table-action-view" title="Show Product">
                                ...
                                <!-- Delete Product -->
                                <button @click="openDeleteModal(product)" class="theme-table-action-btn theme-table-action-delete" title="Delete Product">
                                    <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <ConfirmModal :show="isModalOpen" title="Delete Product" message="Are you sure you want to permanently remove this product from the system?" confirm-label="Yes, Delete Product" cancel-label="No, Keep Product" :badge="selectedProduct?.name" :badge-initial="selectedProduct?.name?.slice(0, 1)" @close="closeModal" @confirm="confirmDelete" />
    </AuthenticatedLayout>
</template>
