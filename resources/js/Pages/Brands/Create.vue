...
// Only auto-fill while creating, or while the user hasn't manually edited the slug.
const slugTouched = ref(isEditing.value); // if editing, treat slug as already "touched" so we don't overwrite it
        ...
        <div class="max-w-8xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight dark:text-slate-100">
                    {{ isEditing ? 'Edit Brand Account' : 'Create New Brand' }}
                </h2>
            </div>
            <Link
        ...
        <div class="max-w-8xl mx-auto pb-24">
            <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div class="flex flex-col">
                                ...
                                <label class="inline-flex items-center gap-3 mt-2 cursor-pointer select-none">
                                    <button
                                        ...
                                        :class="form.status ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-600'"
                                    >
                                        <span
                ...
                <div class="flex items-center justify-center pt-4">
                    ...
                    >
                        {{ isEditing ? 'Update Brand Record' : 'Create Brand' }}
                    </PrimaryButton>
                </div>
            </form>
        </div>
