...
const form = useForm({
    head_code: props.expenseHead?.head_code ?? '',
    name: props.expenseHead?.name ?? '',
    description: props.expenseHead?.description ?? '',
    status: props.expenseHead?.status ?? true,
});

const submit = () => {
    form.put(route('expense-heads.update', props.expenseHead.id));
};
</script>

<template>
    <Head :title="$t('Edit Expense Head')" />

    <AuthenticatedLayout>
        <div class="max-w-8xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight dark:text-slate-100">{{ $t('Edit Expense Head') }}</h2>
                <p class="text-sm text-slate-800 mt-1 font-medium dark:text-slate-400">{{ $t('Update existing expense head details.') }}</p>
            </div>
                    ...
                    <div class="p-8 md:p-10">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                            <div class="flex flex-col">
                                ...
                                <TextInput id="head_code" type="text" class="theme-form-input" v-model="form.head_code" required />
